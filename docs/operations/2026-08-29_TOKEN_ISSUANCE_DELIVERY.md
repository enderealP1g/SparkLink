...
- Portal landing page 已通过真实公网浏览器登录；页面显示 `root (usr_plus_manual_01) · OWNER`、Plan `Plus`、`legacy-pre-baseline` Customer Cycle（`Asia/Shanghai`），并独立显示 `STANDARD`/`ADVANCED`/`PREMIUM` pool rows。尚无 Advanced runtime entry 的环境仍会将其 Usage/Subscription coverage 标为 `Unknown` 或 `not_configured`，不会伪造 traffic。root token 未进入聊天、日志或页面响应输出。
