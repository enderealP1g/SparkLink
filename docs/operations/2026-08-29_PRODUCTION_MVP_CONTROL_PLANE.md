...
| Field | Value |
| --- | --- |
| Status | QQG origin operational；public Cloudflare cutover pending |
...
Windows manual collector --SSH read-only--> Xray Stats API on Nodes
                 ...
                 Nginx TLS path: hypro02-cdn:2053/sparklink-mvp/
                                      |
                        future Cloudflare Worker boundary
...
| Area | Evidence |
| --- | --- |
| Code | local `compileall` + 7 unit tests passed；QQG remote test run 7/7 passed |
...
| Usage | 当前 manual Plus User 的 `STANDARD` coverage 为 `available`、used 为 `0`；`PREMIUM` coverage 为 `available`、used 为 `611798` bytes；total 为 `611798` bytes |
| HTTP origin | Nginx TLS path 的 Portal、Bearer `/api/me`、Bearer `/subscription` 均返回成功；subscription 为 6 行、全部 `vless` |
| Client paths | 两条 hypro02 Xray/VLESS REALITY path 产生实际请求并形成 non-zero Stats counters；无 key probe 的 OpenAI/Anthropic 返回 `401`、Gemini `403`、Google AI `200`、Google `204`。此前 Gate B 的 Native policy-level `403` 负面 evidence 仍保留，不能被这些 no-key probes 覆盖。 |
| Reboot | reboot 后历史 ledger 仍为 `611798` bytes；new counter epoch 重新可采集，未清零历史 Usage |

## Public edge status

QQG origin path 已准备 `/sparklink-mvp/` reverse proxy，并保留原 CDN VLESS exact path。新的 edge Worker source 位于 [`cloudflare/sparklink-edge-worker.js`](../../cloudflare/sparklink-edge-worker.js)，但本次未伪造 Cloudflare deployment：当前 control plane 未发现可用的 Cloudflare API credential/CLI session，`spark.enrpiglink.top` 与 `sub.enrpiglink.top` 的 public binding 仍需受保护的人工或授权 control-plane 操作。

完成 public cutover 前，不得把 origin smoke 描述为用户已通过公网 Portal 验收。所需最小人工动作是：在 Cloudflare zone 中 review/deploy Worker、绑定已有/确认后的 delivery boundary、配置 `CONTROL_PLANE_ORIGIN`、hostname-scoped Strict TLS/origin port/cache bypass，并从隔离 client 重新验证；不要改动其他 production records。
...
- 无 Cloudflare credential 时，管理员可先使用受保护 SSH/origin path 完成 migration 和 verification；这不等同于 public production cutover。
